...
const IdeaCard = forwardRef(function IdeaCard({idea, handleDelete, handleUpdate}: IdeaProps, forwardedRef) {
    ...
    <Card variant="outlined" sx={{ width: 275, height: 300 }}>
      <CardContent>
        <Typography variant="h2" sx={{ fontSize: 20 }} gutterBottom onClick={() => setEditTitle(true)}>
          {editTitle ? <Input ref={forwardedRef} autoFocus type="text" name="title" onChange={(e) => updateTitle(e.target.value)} onBlur={(e) => setEditTitle(false)} defaultValue={title} /> : title}
        </Typography>
...
// const IdeaCard = ({idea, handleDelete, handleUpdate}: IdeaProps) => {
//   const [title, setTitle] = useState(idea.title);
//   const [editTitle, setEditTitle] = useState(false);
//   const [desc, setDesc] = useState(idea.description);
//   const [editDesc, setEditDesc] = useState(false);
